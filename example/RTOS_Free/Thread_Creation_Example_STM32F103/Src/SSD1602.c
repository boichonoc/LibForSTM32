...
void sendCommand(I2C_HandleTypeDef *hi2c, uint8_t command_s)
{
    temp[0] = COMAND;
    temp[1] = command_s;
    HAL_I2C_Master_Transmit(hi2c,OLED_adress, temp,2,100);
}
//-------------------------Send Data------------------------------------------//
//----------------------------------------------------------------------------//
...
void LCD_Goto(I2C_HandleTypeDef *hi2c, unsigned char x, unsigned char y)
{
    //LCD_X = x;
    //LCD_Y = y;
    sendCommand(hi2c, 0xB0 + y);
    sendCommand(hi2c, x & 0xf);
    sendCommand(hi2c, 0x10 | (x >> 4));
}
//-------------------------Clear Display--------------------------------------//
//----------------------------------------------------------------------------//
...
void LCD_Char(I2C_HandleTypeDef *hi2c, unsigned int c)
{
    ...
    temp_char[6] = 0;
    HAL_I2C_Master_Transmit(hi2c, OLED_adress, temp_char, 7,1000);
    LCD_X += 8;
    if(LCD_X>OLED_WIDTH)
    {
        LCD_X = OLED_DEFAULT_SPACE;
...
void OLED_string(I2C_HandleTypeDef *hi2c, char *string)
{
    while(*string != '\0')
    {
        LCD_Char(hi2c, *string);
        string++;
    }
}
//-------------------------Set num string-------------------------------------//
//----------------------------------------------------------------------------//
void OLED_num_to_str(I2C_HandleTypeDef *hi2c, unsigned int value, unsigned char nDigit)
{
    switch(nDigit)
    {
        case 5: LCD_Char(hi2c, value/10000+48);
        case 4: LCD_Char(hi2c, (value/1000)+48);
        case 3: LCD_Char(hi2c, (value/100)+48);
        case 2: LCD_Char(hi2c, (value/10)+48);
        case 1: LCD_Char(hi2c, value%10+48);
    }
}
//-------------------------Init Display---------------------------------------//
...
{
	// Turn display off
	sendCommand(hi2c, OLED_DISPLAYOFF);

	sendCommand(hi2c, OLED_SETDISPLAYCLOCKDIV);
	sendCommand(hi2c, 0x80);

	sendCommand(hi2c, OLED_SETMULTIPLEX);
	//sendCommand(0x1F);//128x32
	sendCommand(hi2c, 0x3F);//128x64
		
	sendCommand(hi2c, OLED_SETDISPLAYOFFSET);
	sendCommand(hi2c, 0x00);
		
	sendCommand(hi2c, OLED_SETSTARTLINE | 0x00);//0
		
	// We use internal charge pump
	sendCommand(hi2c, OLED_CHARGEPUMP);
	sendCommand(hi2c, 0x14);
		
	// Horizontal memory mode
	sendCommand(hi2c, OLED_MEMORYMODE);
	sendCommand(hi2c, 0x00);
		
	sendCommand(hi2c, OLED_SEGREMAP | 0x1);

	sendCommand(hi2c, OLED_COMSCANDEC);

	sendCommand(hi2c, OLED_SETCOMPINS);
	//sendCommand(0x02);//128x32
	sendCommand(hi2c, 0x12);//128x64
	
	// Max contrast
	sendCommand(hi2c, OLED_SETCONTRAST);
	//sendCommand(0x0F);//0xCF
	sendCommand(hi2c, 0xCF);//0xCF

	sendCommand(hi2c, OLED_SETPRECHARGE);
	sendCommand(hi2c, 0xF1);

	sendCommand(hi2c, OLED_SETVCOMDETECT);
	//sendCommand(0x10);//0x40
	sendCommand(hi2c, 0x40);//0x40

        sendCommand(hi2c, OLED_DISPLAYALLON_RESUME);

	// Non-inverted display
	sendCommand(hi2c, OLED_NORMALDISPLAY);

	// Turn display back on
	sendCommand(hi2c, OLED_DISPLAYON);
}
